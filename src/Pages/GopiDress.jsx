...
export default function DesignerSuit({ addToCart, category = "gopidress", title = "Designer gopiDresses" }) {
  ...
  const handlePrevImage = () => setActiveImageIndex((prev) => prev === 0 ? (modal.images?.length - 1) || 0 : prev - 1);
  const handleNextImage = () => setActiveImageIndex((prev) => prev === (modal.images?.length - 1 || 0) ? 0 : prev + 1);
    ...
    <>
      ...
      <section className="relative h-[75vh] w-full overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-center h-full text-center text-white px-6">
          <div>
            <h1 className="text-4xl md:text-6xl font-[Great_Vibes] text-[#FFD700] drop-shadow-lg mb-4">{title} Collection</h1>
      ...
      {/* GALLERY */}
      <section ref={sectionRef} className="py-16 px-4 md:px-12 bg-gradient-to-b from-[#fff9f5] via-[#fff4ea] to-[#ffeedd]">
        <div className="text-center mb-14">
          <h2 className="text-5xl font-[Great_Vibes] text-[#B22222] mb-3">Elegance in Every Stitch</h2>
          <p className="text-[#5A3E36] text-lg italic max-w-xl mx-auto">"Each piece is a celebration of heritage, artistry, and grace."</p>
            ...
            >
              ...
              <div className="px-2 py-2 flex flex-col justify-between h-[140px]">
                <div>
                  ...
                  <p className="text-sm text-gray-700">{item.description?.split(" ").slice(0, 10).join(" ")}{item.description?.split(" ").length > 10 && "..."}</p>
                </div>
      ...
      {/* MODAL */}
      {modal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[9999] flex items-center justify-center p-4" onClick={() => setModal(null)}>
          <div onClick={(e) => e.stopPropagation()} className="bg-white w-full max-w-[90vw] md:max-w-[70vw] lg:max-w-[65vw] h-[90vh] rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row relative border border-[#e6c17b]">
            <button onClick={() => setModal(null)} className="absolute top-4 right-4 text-[#B22222] text-4xl font-bold hover:text-[#98131f] z-20">×</button>

            <div className="w-full md:w-1/2 h-[40vh] md:h-full relative bg-[#faf7f2] flex items-center justify-center">
              {modal.images?.length ? (
                <>
                  <button onClick={handlePrevImage} className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/70 rounded-full p-2 z-10 hover:bg-white">‹</button>
                  <img src={modal.images[activeImageIndex]} alt="" className="w-full h-full object-contain" />
                  <button onClick={handleNextImage} className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/70 rounded-full p-2 z-10 hover:bg-white">›</button>
                </>
              ) : (
            ...
            <div className="w-full md:w-1/2 h-full p-8 flex flex-col overflow-y-auto hide-scrollbar bg-[#fffaf5]">
              ...
              <a href="https://www.whatsapp.com/catalog/919811676755/?app_absent=0" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-base font-bold text-white bg-gradient-to-r from-[#B22222] to-[#FFD700] px-6 py-3 rounded-full mb-6 hover:from-[#98131f] hover:to-[#f0c243] transition">
              ...
              <div className="space-y-3 text-sm text-[#333] font-medium mb-6">
                {Object.keys(modal).map((key) =>
                  !["_id", "title", "description", "price", "image", "images"].includes(key) && (
                    <p key={key}><span className="font-bold text-[#B22222]">{key}:</span> {modal[key]}</p>
                  )
                )}
              </div>

              <div className="flex gap-4 mt-auto">
                <button onClick={() => handleAddToCart(modal)} className="flex-1 bg-gradient-to-r from-[#B22222] to-[#FFD700] text-white px-6 py-3 rounded-full font-bold hover:from-[#98131f] hover:to-[#f0c243] transition">Add to Cart</button>
                <button onClick={() => handleBuyNow(modal)} className="flex-1 bg-[#FFD700] text-[#B22222] px-6 py-3 rounded-full font-bold hover:bg-[#f0c243] transition">Buy Now</button>
              </div>
            </div>
          </div>
        </div>
