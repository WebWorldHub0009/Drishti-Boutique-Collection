    ...
    <>
      ...
      <section className="relative h-[80vh] w-full overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-center h-full text-center text-white px-6">
          <div>
            <motion.h1 className="text-4xl md:text-6xl font-[Great_Vibes] text-[#D4AF37] mb-4"
            ...
            <motion.p className="max-w-2xl mx-auto text-lg md:text-xl text-white/90 mb-6"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            <motion.button whileHover={{ scale: 1.05 }} onClick={scrollToSection}
      ...
      <section ref={sectionRef} className="py-16 px-4 md:px-12 bg-[#FAFAF0] overflow-hidden">
        <div className="text-center mb-10">
          <h2 className="text-5xl font-[Great_Vibes] text-[#B22222] mb-3">Explore Our {title} Collection</h2>
          <p className="italic text-[#444]">"Gentle on skin, divine in fragrance."</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
          {soaps.map(item => (
            <motion.div key={item._id} onClick={() => setModal(item)}
            ...
            >
              ...
              <div className="p-4">
                ...
                <button onClick={e => { e.stopPropagation(); setModal(item); }}
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[9999] flex items-center justify-center p-4" onClick={() => setModal(null)}>
          ...
          >
            <button onClick={() => setModal(null)} className="absolute top-4 right-4 text-[#B22222] text-4xl font-bold hover:text-[#98131f] z-20">&times;</button>
              ...
              <Swiper navigation pagination={{ clickable: true }} modules={[Navigation, Pagination]} className="h-full w-full">
                {(modal.images || [modal.image]).map((img, idx) => (
                  <SwiperSlide key={idx} className="flex items-center justify-center">
                    <img src={img} alt={`${modal.title}-${idx}`} className="max-w-full max-h-full object-contain" />
                  </SwiperSlide>
                ))}
              </Swiper>
            </div>
            ...
            <div className="w-full md:w-1/2 h-full p-6 md:p-8 flex flex-col overflow-y-auto hide-scrollbar bg-[#fffaf5]">
              ...
              <div className="flex gap-4 mb-6">
                <button onClick={() => handleAddToCart(modal)} className="flex-1 bg-gradient-to-r from-[#B22222] to-[#FFD700] text-white px-6 py-3 rounded-full font-bold hover:from-[#98131f] hover:to-[#f0c243] transition">
                  <FaShoppingCart className="inline-block mr-2" /> Add to Cart
                </button>
                <button onClick={() => handleBuyNow(modal)} className="flex-1 bg-[#FFD700] text-[#B22222] px-6 py-3 rounded-full font-bold hover:bg-[#f0c243] transition">
                  Buy Now
                </button>
              </div>

              <a href="https://www.whatsapp.com/catalog/919811676755/?app_absent=0" target="_blank" rel="noopener noreferrer"
              ...
              <div className="space-y-4 text-sm md:text-base text-[#333] font-medium leading-relaxed mt-auto">
                {Object.entries(modal).filter(([key, value]) => value && !["_id", "title", "description", "price", "image", "images"].includes(key))
                  .map(([key, value]) => (
                    <p key={key}><span className="font-bold text-[#B22222]">{key.replace(/([A-Z])/g, " $1").replace(/^./, str => str.toUpperCase())}:</span> {value}</p>
                  ))
                }
              </div>
            </div>
